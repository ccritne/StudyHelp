                ...
                if studySlotsInfo is not None:
                    window["previewDeadline"].update(
                        value=getStringDate(
                            calculateDeadline(
                                isBook=isBook,
                                arrSessionWeek=studySlotsInfo,
                                totalPages=numberPages,
                                studiedPages=studiedPages,
                            )
                        )
                    )
            else:
...
def creationEvents(
    ...
    while iter <= days:
        if (
            studySlotsInfo[WEEKDAYS[indexWeek]]["isStudyDay"]
            and studySlotsInfo[WEEKDAYS[indexWeek]]["areThereSessions"]
        ):
            ...
            for x in range(studySlotsInfo[WEEKDAYS[indexWeek]]["amount"]):
                ...
                sql += (
                    "('" + studyType + "', '" + insertDate + "', '" + iterDayStr + "'"
                )
        ...
        while iter <= days:
            if studySlotsInfo[WEEKDAYS[indexWeek]]["areThereLectures"]:
                sql += (
                    ...
                    + insertDate
                    + "','"
                    + getStringDate(iterDay)
                    + "', '"
                    + studySlotsInfo[WEEKDAYS[indexWeek]]["timeLectures"][
...
def updateSource(command: str = "NEW"):
    ...
    if command == "MODIFY":
        ...
        for x in range(7):
            if (
                studySlotsInfo[WEEKDAYS[x]]["isStudyDay"]
                and studySlotsInfo[WEEKDAYS[x]]["areThereSessions"]
            ):
    ...
    layout = [
        [
            sg.Text("Name "),
            sg.Input(key="nameDeck", default_text=defaultName, expand_x=True),
        ],
        [
            sg.Checkbox(
                "Associate with lectures?",
                default=withLectures,
                key="checkboxLectures",
                enable_events=True,
            ),
            sg.Text(
                "Name course: ", key="DISPLAY_TEXT_NAME_COURSE", visible=withLectures
            ),
            sg.Input(
                key="INPUT_TEXT_NAME_COURSE",
                default_text=defaultNameCourse,
                visible=withLectures,
            ),
            sg.Button("Insert slots", disabled=not withLectures, key="btnSlots"),
        ],
        ...
        [
            sg.Text(
                "Number of pages ", size=(20, 1), visible=isBook, key="textNumberPages"
            ),
            sg.Input(
                key="numberPages",
                default_text=defaultTotalPages,
                justification="center",
                size=(4, 1),
                enable_events=True,
                visible=isBook,
        ...
        [
            sg.Text(
                ...
                size=(20, 1),
                visible=isBook,
                key="textStudiedPages",
            ),
            sg.Input(
                key="entryStudiedPages",
                default_text=defaultStudiedPages,
                justification="center",
                size=(4, 1),
                enable_events=True,
                visible=isBook,
        ...
        [
            ...
            sg.Input(key="pathSource", size=(10, 1), default_text=defaultPathfile),
            sg.FileBrowse(
                file_types=(("Portable Document Format", "PDF"),), visible=isBook
            ),
        ],
        [
    ...
    window = sg.Window(
        windowTitle, layout=layout, modal=True, finalize=True, keep_on_top=True
    )
    ...
    while True:
        ...
        if event is not None:
            ...
            if event == "STUDY_SLOTS":
                ...
                if sumOfAmount > 0:
                    if command == "NEW":
                        studySlotsInfo = getStudySlots(
                            isBook=isBook,
                            isVideo=isVideo,
                            studyDays=bookStudyDays,
                            arrAmount=arrAmounts,
                            defaults=studySlotsInfo,
                        )
                    if command == "MODIFY":
                        studySlotsInfo = getStudySlots(
                            isBook=isBook,
                    ...
                    for x in range(7):
                        if (
                            ...
                            maxStudyHour = getSettingsValue("maxStudyHour")
                            totalMinutes = (
                                getTotalMinutes(x, exceptID=getSelectedSourceID())
                                + studySlotsInfo[WEEKDAYS[x]]["totalDuration"]
                            )
                            remainingMinutes = maxStudyHour * 60 - totalMinutes
            ...
            if event == "updateSource":
                ...
                if isBook:
                    ...
                    if checkStrIntInput(numberPagesStr) and checkStrIntInput(
                        studiedPagesStr
                    ):
                        numberPages = int(numberPagesStr)
                        studiedPages = int(studiedPagesStr)
            ...
            if event == "btnSlots":
                ...
                if studySlotsInfo["withLectures"]:
                    studySlotsInfo["weekRepsLectures"] = sourceSlotsInfo[
                        "weekRepsLectures"
                    ]
                    studySlotsInfo["startDateLectures"] = sourceSlotsInfo[
                        "startDateLectures"
                    ]
                    studySlotsInfo["endDateLectures"] = sourceSlotsInfo[
                        "endDateLectures"
                    ]
                    for x in range(7):
                        ...
                        studySlotsInfo[WEEKDAYS[x]][
                            "areThereLectures"
                        ] = sourceSlotsInfo[WEEKDAYS[x]]["areThereLectures"]

                        if (
                            WEEKDAYS[x] in sourceSlotsInfo
                            and sourceSlotsInfo[WEEKDAYS[x]]["areThereLectures"]
                        ):
                            studySlotsInfo[WEEKDAYS[x]]["timeLectures"] = {
                                "timeStartDateLecture": sourceSlotsInfo[WEEKDAYS[x]][
                                    "timeLectures"
                                ]["timeStartDateLecture"],
                                "timeEndDateLecture": sourceSlotsInfo[WEEKDAYS[x]][
                                    "timeLectures"
                                ]["timeEndDateLecture"],
                                "durationLecture": sourceSlotsInfo[WEEKDAYS[x]][
                                    "timeLectures"
                                ]["durationLecture"],
                            }
