...
def calculate_deadline(
    ...
    It's created when a new source is inserted and his structure is: \n
    {
        ...
        for x of WEEKDAYS: \n
            x:{ \n
                ...
                'types': list, # His size is the amount, an element can be one of these ['Reading', 'Schematization', 'Testing'] \n
                'durations': list, # His size is the amount, an element is an int between 0 and 59 (the lenght in minutes of the session) \n
                'pages': list, # His size is the amount, an element is an int greater or equal 0 and corresponds to the pages of each sessions \n
    ...
    ##### @param `is_book`: `bool = True`

    With a default value of True, this parameter checks if the source is a book or other (video, audio).

    ##### @param `total_pages`: `int | None = 0`

    Number of pages needed to complete the source.

    ##### @param `studied_pages`: `int | None = 0`
    ...
    #
    # ! ANSWER (@ccritne):
    # It's the total pages (or minutes) of source during the week.
    # In the future index_str can be 'total_pages' or 'total_minutes'.
    # At the moment exists only the book version.
    # list_session_week is explained in the parameter of this function.
    ...
    try:
        ...
        #
        # ! ANSWER (@ccritne):
        # It's the remaining week to complete the book.
        # If it is greater than 0 I sub 1 to have the exact day.
...
def TestDeadline():
    ...
    pages_100_plan_all_in_monday_true_result = calculate_deadline(
        list_session_week=hundread_pages_monday_input, total_pages=100
    )
    if (
