...
def addCard():
    ...
    layout = [
                [
                    ...
                    [sg.Text('Front', size=(10,1)), sg.InputText(key='front', expand_x=True, enable_events=True)],
                    [sg.Text('Back', size=(10,1)), sg.Multiline(key='back', expand_x=True, enable_events=True, size=(100, 8))],
                    [sg.Text('Document', size=(10,1)), sg.InputText(key="filename"), sg.FileBrowse(file_types=(('Image Portable Network Graphics', 'png'),))],
    ...
    while True:
        ...
        if event is not None:
            ...
            if event == "saveNewFlashcard":
                saveNewFlashcard(front=values['front'], back=values['back'], filename=values['filename'])
                break
