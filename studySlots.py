...
def checkAddLectureSlot(
    indexDay: int, hour: int, minute: int, durationMinutes: int, exceptID: int = None
):
    ...
    for x in result:
        ...
        if json[WEEKDAYS[indexDay]]["areThereLectures"]:
            arrStartTime = json[WEEKDAYS[indexDay]]["timeLectures"][
                "timeStartDateLecture"
            ].split(":")
            arrEndTime = json[WEEKDAYS[indexDay]]["timeLectures"][
                "timeEndDateLecture"
            ].split(":")
    ...
    for x in range(7):
        ...
        if defaults != {} and defaults is not None and "withLectures" in defaults:
            if (
                ...
                arrStart = defaults[weekday]["timeLectures"][
                    "timeStartDateLecture"
                ].split(":")
        ...
        row = [
            ...
            sg.Column(
                [
                    [
                        sg.Combo(
                            [fromNumberToTime(x) for x in range(1, 24)],
                            key="START_HOUR_" + (weekday),
                            size=(3, 1),
                            enable_events=True,
                            default_value=defaultStartHour,
                        ),
                        sg.Text(":", size=(1, 1)),
                        sg.Combo(
                            [fromNumberToTime(x) for x in range(0, 60)],
                            key="START_MINUTE_" + (weekday),
                            size=(3, 1),
                            enable_events=True,
                            default_value=defaultStartMinute,
                        ),
                        sg.Text("-", size=(1, 1)),
                        sg.Combo(
                            [fromNumberToTime(x) for x in range(10, 24)],
                            key="END_HOUR_" + (weekday),
                            size=(3, 1),
                            enable_events=True,
                            default_value=defaultEndHour,
                        ),
                        sg.Text(":", size=(1, 1)),
                        sg.Combo(
                            [fromNumberToTime(x) for x in range(0, 60)],
                            ...
                            size=(3, 1),
                            enable_events=True,
                            default_value=defaultEndMinute,
                        ),
                    ]
                ],
        ...
        if event is not None:
            ...
            if (
                ...
                if startHour > endHour:
                    ...
                    window[endHourKey].update(
                        values=[fromNumberToTime(x) for x in range(startHour, 24)],
                        value=fromNumberToTime(min(startHour + 1, 23)),
                    )
                    ...
                    window[startMinuteKey].update(
                        values=[
                            fromNumberToTime(x)
                            for x in range(newMinRangeStart, newMaxRangeStart)
                        ],
                        value=fromNumberToTime(newStartMinute),
                    )
                    window[endMinuteKey].update(
                        values=[
                            fromNumberToTime(x)
                            for x in range(newMinRangeEnd, newMaxRangeEnd)
                        ],
                        value=fromNumberToTime(newEndMinute),
                    )
                else:
                    if startHour == endHour:
                        window[startMinuteKey].update(
                            values=[fromNumberToTime(x) for x in range(0, endMinute)]
                        )
                        ...
                        window[endMinuteKey].update(
                            values=[
                                fromNumberToTime(x)
                                for x in range(newStartMinute + 1, 60)
                            ]
                        )
                        ...
                        window[endHourKey].update(
                            values=[fromNumberToTime(x) for x in range(startHour, 24)],
                            value=fromNumberToTime(endHour),
                        )
                        window[startMinuteKey].update(
                            values=[fromNumberToTime(x) for x in range(0, 60)],
                            value=fromNumberToTime(startMinute),
                        )
                        window[endMinuteKey].update(
                            values=[fromNumberToTime(x) for x in range(0, 60)],
                            value=fromNumberToTime(endMinute),
                        )
                        ...
                        if arrAmount[x] > 0:
                            ...
                            for j in range(arrAmount[x]):
                                ...
                                if sourceID is not None:
                                    oldTotalMinutes = getTotalMinutes(
                                        x, exceptID=sourceID
                                    )
                                ...
                                if (
                                    totalHours > maxStudyHour
                                    and not condErrorHoursIndex
                                ):
                                    condErrorHours = True
